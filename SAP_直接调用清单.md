...
| `ZFI057.vbs` | `ZFI057` | 产值拆分 | ZFI057 workflow 的 step2，按 `ZFIT_RPA_BUKRS-GSBER/WERKS` 映射逐工厂传入上游物料执行拆分；跨月时从截止月份往前回推到最近的发布成本月（1/3/5/7/9/11），按月拆成 1-3 个日期窗口，例如 2026.03.30~2026.04.05 跑 3 月+4 月，2026.06.29~2026.07.05 跑 5 月+6 月+7 月，跨年 1 月回推到上年 11 月后再跑 11 月+12 月+1 月 | 单个 `S_WERKS-LOW`、`S_MATNR` 多选、`S_KADKY-LOW/HIGH`、`S_KADAT-LOW/HIGH` | 依赖 step1 物料清单和 `ZFIT_RPA_BUKRS` 工厂结果；输出 step2 SAP 状态 | VBS 不直接读 `ZFI_SPLIT`；若提示缺物料，回查 NCo step1；若部分窗口无数据，看 `STATUS_TEXT` 中 success/noData |
