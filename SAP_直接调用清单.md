...
| RFC function | `ZFI_SAP_API_GATEWAY` | `Zfi019NlMemoryFetcher.cs`；`Program.cs` 的 ZFI057 workflow | 统一网关：`GET_GS03` 取业务范围对应工厂；`REPORT_SUBMIT` 提交报表并读取 memory/export 结果 | `IV_ACTION`、`IV_JSON_IN`，部分路径尝试 `IV_SET_NAME` | `EV_SUBRC`、`EV_MSG`、`EV_JSON_OUT` | 若失败，按 `EV_SUBRC/EV_MSG`、`IV_ACTION`、JSON 入参结构、ABAP 网关版本排查 |
| RFC function | `RFC_READ_TABLE` | `Zfi019NlMemoryFetcher.cs` | 读取 SAP 表 `TBTCO`、`ZFI_SPLIT` | `QUERY_TABLE`、`OPTIONS`、`FIELDS`、`DELIMITER=|`、`ROWCOUNT=0` | `DATA-WA`，按 `FIELDS` offset/length 拆行 | 重点检查 RFC 权限、字段长度、`OPTIONS` 单行 72 字符限制、日期格式 |
...
### 2.2 `ZFI_SAP_API_GATEWAY` - `GET_GS03`

| 项 | 内容 |
| --- | --- |
| 调用位置 | `SapGs03PlantFetcher.FetchPlantsForBusinessArea`；`Program.cs` 的 `ResolveZfi057PlantsFromSap` |
| 用途 | ZFI057 工作流按业务范围获取工厂清单 |
| RFC/function | `ZFI_SAP_API_GATEWAY` |
| 固定 action | `IV_ACTION = GET_GS03` |
| 关键入参 | `IV_SET_NAME`，默认 set name 为 `Z31`；业务范围以 `TITLE`、`business_area`、`businessArea`、`gsber` 等候选键写入 `IV_JSON_IN` |
| 输出 | `EV_SUBRC`、`EV_MSG`、`EV_JSON_OUT`；从 JSON 中解析 `RT_SET_VALUES`、`SET_VALUES`、`VALUES`、`ROWS`、`DATA`、`ET_LINES` 等候选数组 |
| 依赖 | NCo 连接完整；ABAP 网关支持 `GET_GS03` action；返回行中可解析工厂/业务范围映射 |
| 排查建议 | 若返回空，先确认 `setName`、业务范围入参、网关版本；若 RFC 抛错，查 NCo 连接和函数授权；若解析为空，保存脱敏后的返回结构字段名给 ABAP 侧确认 |
...
| 触发 | 请求业务范围命中特殊范围，或 ALV 中业务范围命中特殊范围 |
| 特殊范围默认值 | `0162, 7700, 7800, 7600, 1070, 0500, 7900`，可由 `dongtaiBusinessAreas/specialBusinessAreas` 覆盖 |
| 排查建议 | 若物料缺失，先确认 `S_BUDAT/BUDAT` 是明确 include 日期范围；再查 `BUKRS/WERKS`、表授权、特殊范围配置 |
...
| `ZFI057.vbs` | `ZFI057` | 产值拆分 | ZFI057 workflow 的 step2，按业务范围映射工厂并传入上游物料执行拆分 | `S_WERKS` 单值或多选、`S_MATNR` 多选、`S_KADKY-LOW/HIGH`、`S_KADAT-LOW/HIGH` | 依赖 step1 物料清单和 `GET_GS03` 工厂结果；输出 step2 SAP 状态 | VBS 不直接读 `ZFI_SPLIT`；若提示缺物料，回查 NCo step1；若部分窗口无数据，看 `STATUS_TEXT` 中 success/noData |
...
| `ZFI019NA.vbs` | `ZFI019NA` | 周损益生成 | 按工厂和上一完整周过账日期生成/保存周损益 | `S_BUDAT-LOW/HIGH`、`S_WERKS-LOW` | SAP 状态栏/`STATUS_TEXT` | 多工厂应由父任务拆批；先查工厂、日期、SAP 状态栏 |
| `ZFIR034.vbs` | `ZFIR034` | 维护特殊价格（ZFI085） | 按日期范围和周次执行报表/维护流程 | `P_WEEK`、`S_BUDAT-LOW/HIGH` | SAP 状态栏/`STATUS_TEXT` | 不使用工厂或业务范围；若周次不对，查系统日期和 `period/weekEnd` |
| `ZFI019NL.vbs` | `ZFI019NL` | 周损益保存导出 | 按业务范围和过账日期提取周损益 ALV 并执行原保存动作 | `S_BUDAT-LOW/HIGH`、`S_GSBER-LOW`；可用 `ALV_EXPORT_DIR/FILENAME` | 先导出业务范围原始 ALV，后端再按 Excel 中 `WERKS`/`Plant Code`/`工厂`/`工厂号`/`工厂代码` 等工厂字段拆分为 `<alvExportDataDirectory>\yyyy_WKnn_工厂` 下的工厂级 Excel；`MATERIAL_COUNT`、`MATERIALS_CSV`、SAP 状态栏；特殊业务范围提示需后端读 `ZFI_SPLIT` | 若物料为空，查 ALV 列 `MATNR/MATNR1/RMATNR/IMATNR`；若拆分失败，先确认导出 Excel 是否有工厂列及列值，不允许退回 `ZTSD001`、SQLite、本地配置或业务范围入参推断；特殊范围补料要回查 NCo `ZFI_SPLIT` |
...
5. Step 2：NCo 调用 `ZFI_SAP_API_GATEWAY`，`IV_ACTION=GET_GS03`，按业务范围解析工厂。
6. Step 2：launcher 把物料清单外置给 `ZFI057.vbs`，VBS 在 SAP GUI 中填 `S_WERKS`、`S_MATNR`、`S_KADKY`、`S_KADAT` 并执行。
...
| 有 ZFI019NL 数据但 ZFI057 无工厂 | `GET_GS03` 的 `setName`、业务范围、返回 JSON 结构 |
