...
当前联调默认通知人为 `11464769`，但只在执行页勾选“勾选固定通知 11464769”时使用。取消勾选后必须使用 URL token 里的 `Account`；如果 token 解析失败或缺少 `Account`，页面禁止提交执行。
...
- 页面没有账号/密码或钉钉扫码登录入口，对方平台图标点击后直接跳转 URL。
- 识别到合法 `Account` 后，页面会直接进入执行任务页，顶部身份位置显示该员工号，自动取消“勾选固定通知 11464769”，当前提交用户改为 token 中的 `Account`。
- 用户重新勾选固定通知后，会切回 `11464769`。
- 未勾选固定通知且没有可用 `Account` 时，提交按钮会被禁用，前端不会触发 `/api/runs`。
...
5. 正式 SAP RPA 页面应直接进入执行任务页，确认固定通知复选框自动取消，当前提交显示为 token 中的 `Account`。
6. 不勾选固定通知时提交任务，检查本地 API 请求或钉钉通知目标，应该使用该 `Account`。
7. 重新勾选固定通知后提交任务，通知目标应切回 `11464769`。
8. 用缺少 `Account` 的 token 或普通 URL 打开页面后取消勾选固定通知，提交按钮应禁用或提示“未解析到 token 员工号”，不能创建 run。
