...
If Trim(CStr(tcode)) = "" Or Trim(CStr(tcode)) = unresolvedOkCodeToken Then tcode = "ZFI080B"
If UCase(Trim(CStr(tcode))) <> "ZFI080B" Then Fail "ZFI080B script refuses tcode=" & CStr(tcode), 10
...
If plantValue = "" Then Fail "ZFI080B requires one plant from {PLANTS}", 5
...
Sub ResolveDates()
   ...
   If DateDiff("d", parsedStart, parsedEnd) < 0 Then Fail "ZFI080B weekEnd is before period", 5
...
WScript.Echo "INFO: date input group #1; S_BUDAT=[" & dateLowValue & "~" & dateHighValue & "]"
...
session.findById("wnd[0]/usr/ctxtS_BUDAT-HIGH").caretPosition = Len(dateHighValue)
Err.Clear
PressExecute
WaitReady 1200000
SelectAllGrid
ExportAlvBeforeSave "save selected rows", ""
PressToolbarButton "wnd[0]/tbar[1]/btn[16]", "save/export selected rows"
